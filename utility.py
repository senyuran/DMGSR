...
class checkpoint():
    ...
    def save_results(self, filename, save_list, scale, sig, dataset):
        def _make_dir(path):
            if not os.path.exists(path): os.makedirs(path)

        filename = '{}/results/x{}/{}/sig{}_{}'.format(self.dir, scale, dataset, sig, filename)
        _make_dir(self.dir + '/results/' + 'x{}/{}'.format(scale, dataset))
        normalized = save_list[0][0].data.mul(255 / self.args.rgb_range)
        ndarr = normalized.byte().permute(1, 2, 0).cpu().numpy()
        imageio.imwrite('{}.png'.format(filename), ndarr)

    def save_res(self, filename, kv):
        np_images = kv.cpu().numpy()
    # 遍历96张图片并保存
        for i in range(96):
            filename = '{}_x{}_'.format('res', i)
            # 将图片归一化到0-255
            image = (np_images[0, i] * 255).astype(np.uint8)
            # 使用PIL将numpy数组转换为图像
            pil_image = Image.fromarray(image)
            # 保存图像
            # 如果filename已经包含了完整路径，则直接使用它
            pil_image.save(open(f"{filename}.png", "wb"), "PNG")
            # pil_image.save(open(f"{filename}.png", "wb"), "PNG")

    def save_short(self, filename, kv):
        np_images = kv.cpu().numpy()
    # 遍历96张图片并保存
        for i in range(96):
            filename = '{}_x{}_'.format('short', i)
            # 将图片归一化到0-255
            image = (np_images[0, i] * 255).astype(np.uint8)
            # 使用PIL将numpy数组转换为图像
            pil_image = Image.fromarray(image)
